...
function generateDungeon() {
    ...
    for (let tries = 0; tries < 500 && gridRooms.length < targetRooms; tries += 1) {
...
function findEnemyAt(x, y) {
    return state.enemies.find((enemy) => enemy.x === x && enemy.y === y && enemy.roomId === state.currentRoomId)
}

function pickEnemyPattern() {
    if (state.floor < 4) {
        return ['line', 'line', 'rain', 'burst'][randInt(0, 3)]
    }
    if (state.floor < 8) {
        return ['line', 'rain', 'burst', 'spiral', 'spiral', 'pipe'][randInt(0, 5)]
    }
    if (state.floor < 12) {
        return ['line', 'rain', 'burst', 'spiral', 'pipe', 'pipe'][randInt(0, 5)]
    }
    return ['rain', 'burst', 'line', 'spiral', 'pipe', 'enfer'][randInt(0, 5)]
}
...
function resetFloor() {
    ...
    for (const room of nonStartRooms) {
        const roomTiles = getRoomTiles(room).filter((tile) => !occupied.has(`${tile.x},${tile.y}`))
        if (!roomTiles.length) continue
        const mobCount = room.width * room.height >= 42 ? 2 : 1
        for (let i = 0; i < mobCount; i += 1) {
            const options = roomTiles.filter((tile) => !occupied.has(`${tile.x},${tile.y}`))
    ...
    if (nonStartRooms.length) {
        ...
        const tiles = getRoomTiles(farRoom).filter((tile) => !occupied.has(`${tile.x},${tile.y}`))
        if (tiles.length) {
...
function canWalk(x, y) {
    ...
    return x >= 0 && y >= 0 && x < room.width && y < room.height && room.grid[y][x] === TILE_FLOOR
}
...
function processEnemyMapTurn() {
    const roomEnemies = state.enemies.filter((e) => e.roomId === state.currentRoomId)
...
function tryMovePlayer(dx, dy) {
    ...
    else if (nx >= room.width && room.doors.right) newRoomId = room.doors.right.id
...
function spawnPatternProjectiles() {
    ...
    if (pattern === 'burst') {
        ...
        for (let i = 0; i < count; i += 1) {
            ...
            spawnProjectile(randomMode(), cx, cy, Math.cos(angle) * speed, Math.sin(angle) * speed)
        }
        return
    }
    ...
    if (pattern === 'pipe') {
        if (!dodge.pipeZone) {
            dodge.pipeZone = Math.random() < 0.5 ? [0.1, 0.9] : [0.5]
        }
    ...
    if (pattern === 'spiral') {
        ...
        for (let i = 0; i < arms; i += 1) {
            ...
            spawnProjectile(randomMode(), 0.5, 0.5, Math.cos(angle) * speed, Math.sin(angle) * speed, 0.02)
        }
        return
    }
...
function updateEnemyDodge(dt) {
    ...
    let dx = 0
    let dy = 0
    if (state.keysDown.has('arrowleft') || state.keysDown.has('q')) dx -= dodge.speed * dt
    if (state.keysDown.has('arrowright') || state.keysDown.has('d')) dx += dodge.speed * dt
    if (state.keysDown.has('arrowup') || state.keysDown.has('z')) dy -= dodge.speed * dt
    if (state.keysDown.has('arrowdown') || state.keysDown.has('s')) dy += dodge.speed * dt
...
function drawMap() {
    ...
    const TILE = Math.max(1, Math.floor(Math.min(availableWidth / room.width, availableHeight / room.height)))
    const OFFSET_X = Math.floor((availableWidth - room.width * TILE) / 2)
    const OFFSET_Y = HUD_HEIGHT + Math.floor((availableHeight - room.height * TILE) / 2)

    for (let y = 0; y < room.height; y += 1) {
        for (let x = 0; x < room.width; x += 1) {
            const isFloor = room.grid[y] && room.grid[y][x] === TILE_FLOOR
            c.fillStyle = isFloor ? '#1f2230' : '#3f4350'
            c.fillRect(OFFSET_X + x * TILE, OFFSET_Y + y * TILE, TILE, TILE)
        }
    }
    ...
    if (room.doors.up) {
        ...
        c.lineWidth = 2
        c.strokeRect(OFFSET_X + Math.floor(room.width / 2 - 1) * TILE, OFFSET_Y - 4, 2 * TILE, 4)
    }
    if (room.doors.down) {
        c.strokeStyle = '#88ff88'
        c.lineWidth = 2
        c.strokeRect(OFFSET_X + Math.floor(room.width / 2 - 1) * TILE, OFFSET_Y + room.height * TILE, 2 * TILE, 4)
    }
    if (room.doors.left) {
        c.strokeStyle = '#88ff88'
        c.lineWidth = 2
        c.strokeRect(OFFSET_X - 4, OFFSET_Y + Math.floor(room.height / 2 - 1) * TILE, 4, 2 * TILE)
    }
    if (room.doors.right) {
        c.strokeStyle = '#88ff88'
        c.lineWidth = 2
        c.strokeRect(OFFSET_X + room.width * TILE, OFFSET_Y + Math.floor(room.height / 2 - 1) * TILE, 4, 2 * TILE)
    }

    window.currentRoomDisplay = { room, TILE, OFFSET_X, OFFSET_Y }
}

function drawEntities() {
    const display = window.currentRoomDisplay
    if (!display) return
    const { TILE, OFFSET_X, OFFSET_Y } = display

    c.fillStyle = '#5ce38f'
    c.fillRect(OFFSET_X + state.player.x * TILE + 3, OFFSET_Y + state.player.y * TILE + 3, TILE - 6, TILE - 6)

    const roomEnemies = state.enemies.filter((e) => e.roomId === state.currentRoomId)
    for (const enemy of roomEnemies) {
        c.fillStyle = enemy.isBoss ? '#ff1744' : '#ff5f6f'
        c.fillRect(OFFSET_X + enemy.x * TILE + 4, OFFSET_Y + enemy.y * TILE + 4, TILE - 8, TILE - 8)
    }

    if (state.trap && state.trap.roomId === state.currentRoomId) {
        c.fillStyle = '#ffd700'
        c.beginPath()
        const tx = OFFSET_X + state.trap.x * TILE + TILE / 2
        const ty = OFFSET_Y + state.trap.y * TILE + TILE / 2
        c.arc(tx, ty, TILE / 2 - 2, 0, Math.PI * 2)
        c.fill()
    }
}
...
function drawMiniMap() {
    ...
    for (const room of state.rooms) {
        ...
        for (const direction of ['up', 'down', 'left', 'right']) {
            ...
            c.strokeStyle = neighbor.id === state.currentRoomId || room.id === state.currentRoomId ? 'rgba(255, 235, 120, 0.95)' : 'rgba(140, 255, 140, 0.6)'
            c.lineWidth = neighbor.id === state.currentRoomId || room.id === state.currentRoomId ? 3 : 2
            c.beginPath()
...
function drawHud() {
    ...
    const roomEnemyCount = state.enemies.filter((e) => e.roomId === state.currentRoomId).length
    c.fillText(`Ennemis salle ${roomEnemyCount}`, 210, 46)
    if (state.phase === 'explore') c.fillText('ZQSD/Fleches: deplacement', 370, 24)
    if (state.phase === 'explore' && state.trap) c.fillText('Trappe: contact avec circle jaune', 370, 46)
    if (state.phase === 'explore' && !state.trap) c.fillText('Bat le boss pour delocker la trappe', 370, 46)
}
...
function drawBattle() {
    ...
    c.fillText(`Enemy HP: ${state.battle.enemy.hp}/${state.battle.enemy.maxHp}`, 110, 148)
    ...
    if (state.phase === 'battleDodge') {
        ...
        for (const p of dodge.projectiles) {
            c.fillStyle = p.mode === 'white' ? '#ffffff' : p.mode === 'orange' ? '#ff8f34' : '#58b4ff'
            c.beginPath()
            c.arc(box.x + p.x * box.w, box.y + p.y * box.h, p.radius * box.w, 0, Math.PI * 2)
            c.fill()
        }
        c.fillStyle = '#ffffff'
...
function drawEndOverlay(text, subtext) {
    ...
    c.fillText(text, Math.floor(window.innerWidth * 0.33), Math.floor(window.innerHeight * 0.45))
    c.font = '20px monospace'
    c.fillText(subtext, Math.floor(window.innerWidth * 0.28), Math.floor(window.innerHeight * 0.52))
}
...
function render() {
    ...
    if (state.phase === 'battleChoice' || state.phase === 'battleSkill' || state.phase === 'battleDodge') {
        drawBattle()
    }
